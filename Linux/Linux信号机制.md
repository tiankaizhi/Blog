...
以下是 Red Hat 7.8 支持 64 种信号，这里就不详细列出每种信号的具体作用了，想深入了解的小伙伴可以自行查阅相关资料。
...
信号事件主要有两个来源：硬件来源（如键盘或硬件故障）；软件来源，最常用发送信号的系统函数是 ```kill()```、```raise()```、```sigqueue()```、```alarm()```、```setitimer()``` 以及 ```abort()``` 等函数，软件来源还包括一些非法运算等操作。
