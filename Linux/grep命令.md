...
```grep``` （Globally search a Regular Expression and Print）是一种强大的文本搜索工具，它能使用特定模式匹配（包括正则表达式）搜索文本，并默认输出匹配行。Unix 的 ```grep``` 家族包括 ```grep```、```egrep``` 和 ```fgrep```。Windows 系统下类似命令 ```FINDSTR```。
