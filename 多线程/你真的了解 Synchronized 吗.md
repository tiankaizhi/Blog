...
Synchronized 是干嘛的，这里就不介绍了，有不清楚的小伙伴自行百度下哈。前言我觉得还是先把几个比较重要的问题抛出来吧。

1. 工作中经常有听到说 Synchronized 是重量级锁，性能比较差，最好不要用，使用 Lock 锁，比如 ReetrantLock 来代替 Synchronized。

2. Synchronized 使用的是 monitorenter 和 monitorexit 指令实现的。
