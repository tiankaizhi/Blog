...
Kafka 起初是 由 LinkedIn 公司采用 Scala 语言开发的一个 **<font color="#e50e0e">多分区、多副本且基于 ZooKeeper 协调</font>** 的分布式消息系统，现已被捐献给 Apache 基金会。目前 Kafka 已经定位为一个 **分布式流式处理平台**，它以 **<font color="#e50e0e">高吞吐、可持久化</font>** 、**可水平扩展、支持流数据处理** 等多种特性而被广泛使用。目前越来越多的开源分布式处理系统如 Cloudera、Storm、Spark、Flink 等都支持与 Kafka 集成。
